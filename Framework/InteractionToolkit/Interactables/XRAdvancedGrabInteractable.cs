    ...
    namespace Interaction.Toolkit
    {
        ...
        public class XRAdvancedGrabInteractable : XRBaseInteractable
		{
                    ...
                    case XRInteractionUpdateOrder.UpdatePhase.Fixed:
						{
                            ...
                            {
                                //If we have a constraint, constrain current physics velocities
                                if (m_Constraint != null)
                                    m_Constraint.ConstrainPhysics(m_Rigidbody);
                            }
                        }
                        break;
                    ...
                    case XRInteractionUpdateOrder.UpdatePhase.Dynamic:
                        {
                            if (isSelected)
                            {
                                ...
                                //Constrain interactor - set
                            }
                            else
                            {
                                //IF we have a constraint, constrain current position and rotation
                                if (m_Constraint != null)
                                {
                                    Vector3 position = this.transform.position;
                                    Quaternion rotation = this.transform.rotation;
                                    m_Constraint.ConstrainTransform(ref position, ref rotation);
                                }
                            }
                        }
                        break;
            ...
            private void UpdateTarget(float timeDelta)
            {
                ...
                if (m_Constraint)
				{
                    m_Constraint.ConstrainTransform(ref rawTargetWorldPosition, ref rawTargetWorldRotation);
				}
